...
import { Landmark, Clock, FileText, Vote, Shield, Eye, Lock } from 'lucide-react'
...
export function GrandeScene({ seance, institutionName }: GrandeSceneProps) {
    ...
    return (
      <div className="min-h-screen bg-[#0D2B55] flex flex-col items-center justify-center text-white p-12">
        ...
        <div className="flex items-center gap-6 mb-12">
          ...
          <div>
            <h1 className="text-5xl font-bold tracking-tight">{institutionName}</h1>
            <p className="text-2xl text-white/60 mt-2">{seance.instance_config?.nom}</p>
          </div>
        </div>

        {/* Session title */}
        <h2 className="text-4xl font-semibold text-center max-w-4xl leading-tight mb-8">
          {seance.titre}
        </h2>

        {/* Date & location */}
        <div className="flex items-center gap-8 text-2xl text-white/70">
    ...
    return (
      <div className="min-h-screen bg-[#0D2B55] flex flex-col items-center justify-center text-white p-12">
        <div className="flex items-center gap-6 mb-12">
          ...
          <h1 className="text-5xl font-bold">{institutionName}</h1>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#0D2B55] flex flex-col text-white">

      {/* Top bar */}
      <header className="flex items-center justify-between px-12 py-6 border-b border-white/10">
        <div className="flex items-center gap-4">
          <Landmark className="h-8 w-8 text-white/60" />
          <span className="text-xl font-semibold text-white/80">{institutionName}</span>
          <span className="text-xl text-white/40">—</span>
          <span className="text-xl text-white/60">{seance.instance_config?.nom}</span>
        </div>
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-16 py-12">
        ...
        {currentPoint && (
          <>
            <div className="flex items-center gap-4 mb-6">
              <span className="flex h-20 w-20 items-center justify-center rounded-2xl text-4xl font-bold bg-white/10 border border-white/20">
                {currentPoint.position}
              </span>
              <div className="flex items-center gap-3">
                {isVotable ? (
                  <span className="flex items-center gap-2 bg-blue-500/20 border border-blue-400/40 rounded-xl px-6 py-2 text-2xl text-blue-300">
                    <Vote className="h-7 w-7" />
                    Soumis au vote
                  </span>
                ) : (
                  <span className="flex items-center gap-2 bg-white/10 rounded-xl px-6 py-2 text-2xl text-white/60">
                    <Eye className="h-7 w-7" />
                    Information
                  </span>
                )}
                {currentPoint.huis_clos && (
                  <span className="flex items-center gap-2 bg-red-500/20 border border-red-400/40 rounded-xl px-6 py-2 text-2xl text-red-300">
            ...
            {/* Title — THE BIG TEXT */}
            <h2 className="text-6xl font-bold text-center max-w-5xl leading-tight mb-8">
              {currentPoint.titre}
            </h2>

            {/* Description */}
            {currentPoint.description && (
              <p className="text-2xl text-white/60 text-center max-w-4xl leading-relaxed mb-6">
                {currentPoint.description}
              </p>
            )}
            ...
            {isVotable && currentPoint.majorite_requise && currentPoint.majorite_requise !== 'SIMPLE' && (
              <div className="mt-4 bg-amber-500/20 border border-amber-400/40 rounded-xl px-8 py-4">
                <p className="text-2xl text-amber-300 flex items-center gap-3">
        ...
        {!currentPoint && (
          <div className="text-center">
            <FileText className="h-20 w-20 text-white/20 mx-auto mb-6" />
            <p className="text-3xl text-white/40">Aucun point à l&apos;ordre du jour</p>
          </div>
        )}
      </main>

      {/* Bottom bar */}
      <footer className="px-12 py-4 border-t border-white/10 flex items-center justify-between text-white/30 text-lg">
